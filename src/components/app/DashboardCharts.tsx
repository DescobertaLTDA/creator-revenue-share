  ...
  return (
    <div className="space-y-4">
      <div className="bg-white border border-zinc-200 rounded-xl p-5">
        <h2 className="text-sm font-semibold mb-4">Receita por dia (USD)</h2>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={data} margin={{ top: 0, right: 8, left: 0, bottom: 0 }}>
            ...
            />
            <Bar dataKey="receita" name="Receita (USD)" fill="#09090b" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>

      <div className="bg-white border border-zinc-200 rounded-xl p-5">
        <h2 className="text-sm font-semibold mb-4">Views e Alcance por dia</h2>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={data} margin={{ top: 0, right: 8, left: 0, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="views" name="Views" stroke="#09090b" dot={false} strokeWidth={2} />
            <Line type="monotone" dataKey="alcance" name="Alcance" stroke="#a1a1aa" dot={false} strokeWidth={2} strokeDasharray="4 3" />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border border-zinc-200 rounded-xl p-5">
          <h2 className="text-sm font-semibold mb-4">Posts publicados por dia</h2>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={data} margin={{ top: 0, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={GRID} vertical={false} />
              <XAxis dataKey="dia" tick={TICK} axisLine={false} tickLine={false} />
              <YAxis tick={TICK} width={32} allowDecimals={false} axisLine={false} tickLine={false} />
              <Tooltip labelFormatter={(l) => `Dia: ${l}`} contentStyle={{ border: "1px solid #e4e4e7", borderRadius: 8, fontSize: 11 }} />
              <Bar dataKey="posts" name="Posts" fill="#52525b" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-white border border-zinc-200 rounded-xl p-5">
          <h2 className="text-sm font-semibold mb-4">Reações por dia</h2>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={data} margin={{ top: 0, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={GRID} vertical={false} />
              <XAxis dataKey="dia" tick={TICK} axisLine={false} tickLine={false} />
              ...
              <Tooltip formatter={(v: number) => v.toLocaleString("pt-BR")} labelFormatter={(l) => `Dia: ${l}`}
                contentStyle={{ border: "1px solid #e4e4e7", borderRadius: 8, fontSize: 11 }} />
              <Bar dataKey="reacoes" name="Reações" fill="#d4d4d8" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
