...
import {
  ...
  XCircle, Archive, Loader2, AlertTriangle, TrendingUp,
} from "lucide-react";
...
export function MetasPage() {
  ...
  const active    = goals.filter((g) => g.calculated_status === "ACTIVE");
  const completed = goals.filter((g) => g.calculated_status === "COMPLETED");
  const expired   = goals.filter((g) => g.calculated_status === "EXPIRED");
  const archived  = goals.filter((g) => g.calculated_status === "ARCHIVED");
    ...
    <div className={cn("rounded-2xl border bg-card p-5 flex flex-col gap-4", borderColor)}>
      ...
      <div className="space-y-2">
        <div className="flex items-end justify-between gap-2">
          <div>
            <p className="text-xl font-bold tabular-nums">{formatBRL(Number(g.current_amount))}</p>
            <p className="text-xs text-muted-foreground">de {formatBRL(Number(g.target_amount))}</p>
          </div>
        ...
        <div className="flex justify-between items-center text-[11px] text-muted-foreground">
          <span>Faltam {formatBRL(Number(g.remaining_amount))}</span>
